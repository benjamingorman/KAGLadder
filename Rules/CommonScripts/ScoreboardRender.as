...
float drawScoreboard(CPlayer@[] players, Vec2f topleft, CTeam@ team, Vec2f emblem)
{
	...
	GUI::DrawText(getTranslatedString("Username"), Vec2f(bottomright.x - 770, topleft.y), SColor(0xffffffff));
	GUI::DrawText(getTranslatedString("Ping"), Vec2f(bottomright.x - 630, topleft.y), SColor(0xffffffff));
	GUI::DrawText(getTranslatedString("ELO:"), Vec2f(bottomright.x - 560, topleft.y), SColor(0xffffffff));
	...
	for (u32 i = 0; i < players.length; i++)
	{
		...
		GUI::DrawText("" + username, Vec2f(bottomright.x - 770, topleft.y), namecolour);
		GUI::DrawText("" + ping_in_ms, Vec2f(bottomright.x - 630, topleft.y), SColor(0xffffffff));
	...
	if (spectators.length > 0)
	{
		...
		for (u32 i = 0; i < spectators.length; i++)
		{
			...
			if (specx < bottomright.x - 100)
			{
				...
				SColor namecolour = getNameColour(p);
				GUI::DrawText(text, Vec2f(specx, specy), namecolour);
				specx += textdim.x + 10;
			}
			else
			{
